...
internal fun <T, R> UiState<T>.map(transform: (T) -> R): UiState<R> = when (this) {
...
@Composable
internal fun <T, R> UiState<T>.composeMap(transform: @Composable (T) -> R): UiState<R> = when (this) {
    is UiState.Success -> UiState.Success(transform(data))
    is UiState.Error -> UiState.Error(throwable)
    is UiState.Loading -> UiState.Loading()
}
...
@Composable
internal fun <T, R> UiState<T>.composeFlatMap(transform: @Composable (T) -> UiState<R>): UiState<R> = when (this) {
