...
struct MediaViewerTestFixture: View {
    ...
    @State private var media: (any UiMedia)?
    ...
    var body: some View {
        ...
        .task { media = await makeMedia() }
        .fullScreenCover(isPresented: $presented, onDismiss: { dismissed = true }) {
            NavigationStack {
                if let media {
                    MediaViewerScreen(
                        medias: [media], initialIndex: 0, preview: nil,
                        post: withoutPost ? nil : makePost(media: media)
                    )
    ...
    private func makeMedia() async -> any UiMedia {
        ...
        let imageURL = FileManager.default.temporaryDirectory.appendingPathComponent("media-fixture.png")
    ...
    private func makePost(media: any UiMedia) -> UiTimelineV2.Post {
