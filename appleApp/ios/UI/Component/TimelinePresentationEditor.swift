...
public struct TimelinePresentationEditor: View {
    ...
    private let availableIcons: [IconType]
    private let withAvatar: Bool
    private let canUseAvatar: Bool
    private let onWithAvatarChange: (Bool) -> Void
    @Binding var enabled: Bool
    @Binding var filterConfig: TimelineFilterConfig
    private let onEditFilter: () -> Void
    private let showEnabled: Bool
    private let showFilter: Bool
    private let showAppearanceOverrides: Bool
    private let timelineAppearance: TimelineAppearance
    @Binding var appearancePatch: AppearancePatch
    private let behaviorContent: AnyView?
    private let titlePlaceholder: LocalizedStringKey
