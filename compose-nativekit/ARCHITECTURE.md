# Compose NativeKit runtime architecture

## Product definition

Compose NativeKit is a Kotlin Multiplatform runtime with a Compose Runtime authoring frontend and four
...
shared @Composable content
        |
        v
Compose Runtime + NativeKitApplier
        |
        v
typed NativeKitWidgetSystem
        |
        +-- Android View
        +-- Jetpack Compose UI
        +-- UIKit
...
A widget is either a leaf or exposes one `NativeKitChildren` container through `NativeKitWidget.children`.
...
```kotlin
interface TextWidget : NativeKitWidget {
...
`NativeKitWidgetSystem<B>` is an immutable map of `KClass` keys to `(B) -> NativeKitWidget` factories.
`NativeKitRendererPlugin<B>` groups registrations while keeping backend mismatches as compile errors.
...
retain declarative content for reattachment. The Compose host owns its child NativeKit composition with
`DisposableEffect`. Every host disposes composition nodes and callbacks when it leaves its owner.

`NativeKitWidget.dispose` is the single widget cleanup hook.

### Deferred item compositions and lazy collections

`rememberNativeKitSubcompositionFactory` creates independently disposable child compositions which
...
`nativekit-lazy-layout` records interval providers without composing item content. Stable keys drive
...
| --- | --- |
| `nativekit-runtime` | Runtime contracts, applier, lifecycle, Android View/Compose/UIKit/AppKit hosts, frame clocks |
| `foundation` | Four common primitives and their four renderer sets |
| `nativekit-lazy-layout` | Lazy DSL/state/coordinator and four native virtual-list adapters |
| `nativekit-resources-moko` | Optional Moko resource environment, backend-neutral image value, and image renderers |
| `demo/shared` | Shared composition and framework exports |
| `demo/androidApp`, `demo/appleApp` | Native application shells |

`nativekit-runtime` does not depend on Foundation. Foundation depends one-way on runtime, and lazy
...
The runtime and Foundation do not own localization or assets. The optional `nativekit-resources-moko`
...
components need no resource overloads. Images become an opaque `NativeKitImage`; the optional module's
...
Compose NativeKit is not production complete. The next gates are:
