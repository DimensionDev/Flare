import Foundation
import QuartzCore
import ComposeNativeKitLazyBenchmark
#if os(macOS)
import AppKit
...
@MainActor final class ComposeNativeKitDriver: BenchmarkDriver {
...
@MainActor final class BenchmarkRunner {
    ...
    init(container: BenchmarkView) {
        ...
        backend = option("--backend", "nativekit")
    }

    private func makeDriver() -> BenchmarkDriver {
        backend == "nativekit" ? ComposeNativeKitDriver(scenario) : NativeDriver(scenario)
    }
