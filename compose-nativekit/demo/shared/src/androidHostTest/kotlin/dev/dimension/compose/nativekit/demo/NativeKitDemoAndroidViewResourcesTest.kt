...
public class NativeKitDemoAndroidViewResourcesTest {
    @Test
    public fun navigatesTheCatalogAndUpdatesEveryFeaturePage() {
        withAttachedHost { host ->
            ...
            assertEquals("NativeKit resource image", image.contentDescription)
                            ...
                            remember {
                                entryProvider<ReconstructionRoute> {
                                    entry<ReconstructionHome> {
                                        Text(
                                            "Home",
                                            NativeKitModifier(testTag = "reconstruction-home"),
                                        )
                                    }
                                    entry<ReconstructionDetail> { route ->
                                        Text(
                                            route.label,
                                            NativeKitModifier(testTag = "reconstruction-${route.label}"),
                                        )
                                    }
                                }
                            }
        ...
        withAttachedHost(
            createHost = { activity ->
                ...
                activity.supportFragmentManager
                    ...
                    .commitNow()
                createAndroidViewDemoView(activity) as NativeKitAndroidViewHost
            },
        ) { activity, host ->
    ...
    public fun multipleAndroidViewNavigationsDoNotClaimAPrimaryFragment() {
        ...
        controller.setup()
        val firstHost = createAndroidViewDemoView(activity) as NativeKitAndroidViewHost
        val secondHost = createAndroidViewDemoView(activity) as NativeKitAndroidViewHost
    ...
    private fun withAttachedHost(block: (NativeKitAndroidViewHost) -> Unit) {
        withAttachedHost { _, host -> block(host) }
    }

    private fun withAttachedHost(block: (FragmentActivity, NativeKitAndroidViewHost) -> Unit) {
        withAttachedHost(
            createHost = { activity -> createAndroidViewDemoView(activity) as NativeKitAndroidViewHost },
            block = block,
        )
    }

    private fun withAttachedHost(
        createHost: (FragmentActivity) -> NativeKitAndroidViewHost,
        block: (FragmentActivity, NativeKitAndroidViewHost) -> Unit,
    ) {
        val controller = Robolectric.buildActivity(FragmentActivity::class.java)
        val activity = controller.get().apply { setTheme(MaterialR.style.Theme_Material3_DayNight) }
        controller.setup()
...
private fun TrackedAndroidViewNavigationPage(
    ...
    Text(
        text = label,
        modifier = NativeKitModifier(testTag = "lifecycle-$label"),
    )
}
