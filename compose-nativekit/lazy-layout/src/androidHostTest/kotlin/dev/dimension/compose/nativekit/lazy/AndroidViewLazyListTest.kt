...
public class AndroidViewLazyListTest {
            ...
            host.setContent {
                ...
                LazyColumn(modifier = NativeKitModifier.None.fillMaxSize()) {
                    items(
                        ...
                        Text("Item $index", modifier = NativeKitModifier.None.height(40f))
                    }
                }
            }
            layout(host)
            ...
            host.setContent {
                ...
                LazyColumn(modifier = NativeKitModifier.None.fillMaxSize()) {
                    items(
                        ...
                        Text("Item ${index - prependedItemsSnapshot}", modifier = NativeKitModifier.None.height(40f))
                    }
                }
            }
            layout(host)
            ...
            host.setContent {
                LazyColumn(modifier = NativeKitModifier.None.fillMaxSize()) {
                    items(items = items, key = { it }) { item ->
                        Text("Item $item", modifier = NativeKitModifier.None.height(48f))
                    }
                }
            }
            layout(host)
    ...
    public fun itemContentPlacesMultipleRootsAlongTheMainAxis() {
        withHost { host ->
            host.setContent {
                LazyColumn(modifier = NativeKitModifier.None.fillMaxSize()) {
                    item(key = "multiple") {
                        Text("First", modifier = NativeKitModifier.None.height(24f))
                        Text("Second", modifier = NativeKitModifier.None.height(36f))
                    }
                }
            }
        ...
        withHost { host ->
            host.setContent {
                LazyRow(
                    ...
                    item(key = "first") {
                        Text("First", modifier = NativeKitModifier.None.width(40f).height(24f))
                    }
                    item(key = "second") {
                        Text("Second", modifier = NativeKitModifier.None.width(60f).height(24f))
                    }
                }
            }
    ...
    private fun withHost(block: (NativeKitAndroidViewHost) -> Unit) {
    ...
    private fun layout(host: NativeKitAndroidViewHost) {
        shadowOf(Looper.getMainLooper()).idle()
        host.measure(
