...
public class AppleLazyBenchmark {
    ...
    public fun nativeCollectionWorkloads() {
        if (getenv("NATIVEKIT_LAZY_BENCHMARK")?.toKString() != "1") return
        check(!Platform.isDebugBinary) { "Build the benchmarkReleaseTest binary before measuring." }
        val selected = getenv("NATIVEKIT_LAZY_BENCHMARK_SCENARIO")?.toKString()
        val fixtures =
            listOf(
        ...
        fixtures.filter { selected == null || it.name == selected }.forEach { fixture ->
            try {
                benchmark(fixture)
                println("NATIVEKIT_BENCH_COMPLETED,${fixture.name}")
            } catch (error: Exception) {
                val message = error.message.orEmpty().replace('\n', ' ')
                failures += "${fixture.name}: $message"
                println("NATIVEKIT_BENCH_FAILED,${fixture.name},$message")
            }
        }
    ...
    fun content(): NativeKitContent {
        ...
        val items: LazyListScope.() -> Unit = {
            items(
                ...
                if (fixture.cards) {
                    Column(modifier = NativeKitModifier.None.fillMaxWidth().height(extent), spacing = 6f) {
                        Row(spacing = 8f) {
                        ...
                        Text(
                            ...
                            modifier = NativeKitModifier.None.fillMaxWidth(),
                        )
                        Row(spacing = 12f) {
                            Text("Reply 12")
                            Text("Repost 34")
                    ...
                    Text(
                        "Item $savedKey",
                        modifier = if (fixture.horizontal) NativeKitModifier.None.width(extent) else NativeKitModifier.None.height(extent),
                    )
                }
            }
        }
        return {
            if (fixture.horizontal) {
                LazyRow(modifier = NativeKitModifier.None.fillMaxSize(), state = state, spacing = 4f, content = items)
            } else {
                LazyColumn(modifier = NativeKitModifier.None.fillMaxSize(), state = state, spacing = 4f, content = items)
            }
        }
    }
}
...
private fun benchmark(fixture: BenchmarkFixture) {
    println("NATIVEKIT_BENCH_BEGIN,${fixture.name}")
    ...
    try {
        ...
        println("NATIVEKIT_BENCH_VIEWPORT,${host.platform},${fixture.name},390,780")
...
private fun measure(
    ...
    println("NATIVEKIT_BENCH_REUSE,${host.platform},${workload.fixture.name},$operation,${nativeIds.size}")
}
...
private fun emit(
    ...
    println(
        "NATIVEKIT_BENCH,${host.platform},${workload.fixture.name},$operation,$iteration,$nanos," +
