 ...
 *
 * Pass an instance to `NativeKitAndroidViewHost` as its `nativeControllerOwner`; the renderer never
...
public object AndroidViewNavigationRendererPlugin : NativeKitRendererPlugin<AndroidViewBackend> {
...
private class AndroidViewNavigationWidget(
            ...
            model.nativeControllerOwner as? AndroidViewNavigationOwner
                ?: error(
                    "Android View navigation requires AndroidViewNavigationOwner(FragmentActivity). " +
                        "Pass it to NativeKitAndroidViewHost(nativeControllerOwner = ...).",
                )
    ...
    /** Called by [NativeKitNavigationPageFragment] when its container view has been created. */
    fun attachFragment(
        token: String,
        fragment: Fragment,
...
private class ViewEntryRecord(
    ...
    fun createFragment(): Fragment {
        ...
        return NativeKitNavigationPageFragment
            .newInstance(
...
internal class NativeKitNavigationPageFragment : Fragment() {
    ...
    companion object {
        private const val ARG_ADAPTER_ID = "nativekit.navigation.adapter_id"
        private const val ARG_TOKEN = "nativekit.navigation.token"

        fun newInstance(
            adapterId: String,
            token: String,
        ): NativeKitNavigationPageFragment =
            NativeKitNavigationPageFragment().apply {
                arguments =
                    Bundle().apply {
...
private fun Fragment.navigationAdapterId(): String? = arguments?.getString("nativekit.navigation.adapter_id")
