...
import dev.dimension.compose.nativekit.navigation.NavigationBackRequest
...
import dev.dimension.compose.nativekit.navigation.hasSameTopologyAs
import dev.dimension.compose.nativekit.navigation.topology
import kotlinx.coroutines.yield

/** Registers the Navigation3-backed Android Compose renderer for [dev.dimension.compose.nativekit.navigation.NavigationDisplay]. */
public object AndroidComposeNavigationRendererPlugin : NativeKitRendererPlugin<AndroidComposeBackend> {
...
private class AndroidComposeNavigationWidget :
    ...
    private fun applyModel(model: NavigationModel) {
        ...
        // Model delivery is already deferred until after the parent NativeKit apply transaction. Keep
    ...
    private fun shadowEntry(
        ...
        shadowEntry(entry.entry) {
            LaunchedEffect(entry.contentKey, host) {
                // The retained predecessor widget tree covers the first frame. Activate its NativeKit
...
private class ComposeEntryHost(
    entry: ResolvedNavigationEntry,
    nativeControllerOwner: dev.dimension.compose.nativekit.NativeKitNativeControllerOwner?,
    subcompositions: dev.dimension.compose.nativekit.NativeKitSubcompositionFactory,
) {
