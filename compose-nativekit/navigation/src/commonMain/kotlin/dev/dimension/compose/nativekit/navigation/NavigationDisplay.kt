...
/**
 * Displays a Navigation3 back stack through the active NativeKit renderer.
 *
 ...
 * restoration.
 *
 * Entry and decorator content must emit only NativeKit widgets. Using another Compose applier can fail
 * at runtime because [NavEntry] does not retain its composable target in its public type.
 *
 * @throws IllegalArgumentException if the stack shape, content keys, or NativeKit presentation
...
public fun <K : Any> NavigationDisplay(
    backStack: List<K>,
    modifier: NativeKitModifier = NativeKitModifier.None.fillMaxSize(),
...
/**
 * Displays an already decorated Navigation3 entry list through the active NativeKit renderer.
 *
 ...
 * rejects the reserved overlay values. Entry and decorator content must emit only NativeKit widgets.
 *
...
public fun <K : Any> NavigationDisplay(
    ...
    val model =
        NavigationModel(
            ...
            nativeControllerOwner = currentNativeKitNativeControllerOwner(),
        )
    SideEffect {
