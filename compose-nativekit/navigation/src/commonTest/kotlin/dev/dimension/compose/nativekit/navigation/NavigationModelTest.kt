@file:OptIn(ExperimentalNativeKitNavigation::class)
...
public class NavigationModelTest {
    ...
    public fun acceptsEntryProviderAndTypedMetadataDsl() {
        val provider: (TestRoute) -> NavEntry<TestRoute> =
            entryProvider<TestRoute> {
                entry<TestHome> { TestNativeKitScreen() }
                entry<TestEditor>(
                    metadata =
                        metadata {
                            put(
    ...
    @Test
    public fun acceptsANativeKitTargetDecorator() {
...
@Composable
@NativeKitComposable
private fun TestNativeKitScreen() = Unit
