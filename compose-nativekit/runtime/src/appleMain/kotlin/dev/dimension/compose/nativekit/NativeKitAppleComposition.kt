...
public class NativeKitAppleComposition<B : NativeKitBackend>(
...
private object AppleRecomposerRuntimePool {
    ...
    fun acquire(): AppleRecomposerRuntime {
        check(NSThread.isMainThread) {
            "The Apple NativeKit runtime must be acquired on the main thread."
        }
    ...
    fun release(value: AppleRecomposerRuntime) {
        check(NSThread.isMainThread) {
            "The Apple NativeKit runtime must be released on the main thread."
        }
        check(runtime === value && users > 0) {
            "The Apple NativeKit runtime was released without a matching acquisition."
        }
        users -= 1
        if (users == 0) {
            runtime = null
