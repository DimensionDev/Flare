...
COMPOSE_NATIVEKIT = ROOT / "compose-nativekit"
...
def run_logged(command, path, environment=None, allow_workload_failure=False):
    ...
    if result.returncode:
        if allow_workload_failure and result.returncode == 1 and "NATIVEKIT_BENCH_FAILED," in path.read_text():
...
def summarize(output):
    ...
    for path in sorted(output.glob("*-round-*.log")):
        ...
        completed = {line.split(",")[1] for line in lines if line.startswith("NATIVEKIT_BENCH_COMPLETED,")}
        failed = [line.split(",", 2) for line in lines if line.startswith("NATIVEKIT_BENCH_FAILED,")]
        ...
        found = {}
        for line in lines:
            if line.startswith("NATIVEKIT_BENCH,"):
                values = line.split(",")[1:]
            ...
            elif line.startswith("NATIVEKIT_BENCH_REUSE,"):
...
def main():
    ...
    if not args.skip_build:
        run_logged([
            str(ROOT / "gradlew"), "-p", str(COMPOSE_NATIVEKIT), "-PnativeKitLazyBenchmark=true",
            ":nativekit-lazy-layout:linkBenchmarkReleaseTestMacosArm64",
            ":nativekit-lazy-layout:linkBenchmarkReleaseTestIosSimulatorArm64", "--console=plain",
        ], output / "build.log")
    binaries = {
        platform: COMPOSE_NATIVEKIT / "lazy-layout/build/bin" / target / "benchmarkReleaseTest/benchmark.kexe"
    ...
    metadata = {
        ...
        "source_sha256": {
            ...
            for path in sorted((COMPOSE_NATIVEKIT / module / "src").rglob("*")) if path.is_file()
        },
    }
    (output / "metadata.json").write_text(json.dumps(metadata, indent=2) + "\n")
    environment = dict(os.environ, NATIVEKIT_LAZY_BENCHMARK="1", SIMCTL_CHILD_NATIVEKIT_LAZY_BENCHMARK="1")
    environment.pop("NATIVEKIT_LAZY_BENCHMARK_SCENARIO", None)
    environment.pop("SIMCTL_CHILD_NATIVEKIT_LAZY_BENCHMARK_SCENARIO", None)
