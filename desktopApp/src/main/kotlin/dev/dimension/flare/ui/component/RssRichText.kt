...
fun RssRichText(
    ...
    textStyle: TextStyle = FluentTheme.typography.body,
    imageHeader: ImmutableMap<String, String>? = null,
) {
    val uriHandler = LocalUriHandler.current
    val isDarkTheme = FluentTheme.colors.darkMode
    val context =
        remember(element, isDarkTheme) {
            RenderContext(
                RichTextStyle(
                    stringStyle =
                        RichTextStringStyle(
                            linkStyle =
                                TextLinkStyles(
                                    textStyle
                                        .copy(
                                            color = if (isDarkTheme) darkLinkColor else lightLinkColor,
...
@Composable
private fun RichTextFluentTheme(child: @Composable () -> Unit) {
    val isApplied = LocalFluentThemingApplied.current

    if (!isApplied) {
        RichTextThemeProvider(
            textStyleProvider = { FluentTheme.typography.body },
            contentColorProvider = { FluentTheme.colors.text.text.primary },
            textStyleBackProvider = { _, content -> content() },
            contentColorBackProvider = { _, content -> content() },
        ) {
            CompositionLocalProvider(LocalFluentThemingApplied provides true) {
                child()
            }
        }
    } else {
...
private val LocalFluentThemingApplied = compositionLocalOf { false }
...
private fun RichTextScope.RenderElement(
    ...
    when (name) {
        ...
        "h1" -> {
            with(context) {
                RenderTextAndReset(modifier = Modifier.fillMaxWidth())
            }
            Heading(1) {
                element.childNodes().forEach { RenderNode(context, it) }
                with(context) { RenderTextAndReset(modifier = Modifier.fillMaxWidth()) }
            }
        }
