...
def main():
    ...
    variants = [("optimized", "flare", args.optimized_app), ("native", "native", args.optimized_app)]
    if args.baseline_app:
        variants.insert(0, ("before", "flare", args.baseline_app))
    metadata = {
    ...
    metadata["source_diff_sha256"] = hashlib.sha256(subprocess.check_output(["git", "diff", "--", "flareUI/lazy-layout", "flareUI/runtime"])).hexdigest()
    if args.simulator:
