...
public fun UiProfile.toDbUser(host: String = this.host ?: key.host): DbUser =
    DbUser(
        userKey = key,
        name = name.raw,
...
public suspend fun CacheDatabase.upsertUser(user: DbUser) {
    upsertUsers(listOf(user))
}

public suspend fun CacheDatabase.upsertUsers(users: List<DbUser>) {
    if (users.isEmpty()) {
        return
    }
