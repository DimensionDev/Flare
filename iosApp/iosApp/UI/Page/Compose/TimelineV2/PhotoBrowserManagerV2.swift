...
class PhotoBrowserManagerV2 {
    static let shared = PhotoBrowserManagerV2()
        ...
        browser.reloadCellAtIndex = { [weak self] context in
            ...
            case .video:
                   ...
                   let cell = context.cell as? MediaBrowserVideoCell
                {
                    cell.load(
                        url: url,
                        previewUrl: URL(string: media.previewUrl ?? media.url),
                        isGIF: false,
                        headers: headers
                    )
                }
            ...
            case .image:
                if let url = URL(string: media.url),
                   let previewUrl = URL(string: media.previewUrl!),
                        ...
                        case .success:
                            ...
                            cell.imageView.kf.setImage(
                                ...
                                case let .failure(error):
                                    FlareLog.error("PhotoBrowserManagerV2 load original image failed: \(url), error: \(error.localizedDescription)")
                                }
                            }
                        case let .failure(error):
                            FlareLog.error("PhotoBrowserManagerV2 load preview image failed: \(previewUrl), error: \(error.localizedDescription)")
                        }
                    }
                }
            case .audio:
        ...
        PHPhotoLibrary.requestAuthorization { status in
            ...
            case .authorized:
                PHPhotoLibrary.shared().performChanges({
                    ...
                    completion(success)
                    if let error {
                        FlareLog.error("PhotoBrowserManagerV2 save image failed: \(error.localizedDescription)")
                    }
                }
            default:
                completion(false)
                FlareLog.warning("PhotoBrowserManagerV2 no album access permission")
            }
        }
    }
}
