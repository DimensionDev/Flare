...
class FeedTabManager {
    ...
    func moveFeedTab(
        ...
        FlareLog
            .debug(
                "moveFeedTab调用: source=\(source), destination=\(destination)"
            )
        ...
        guard let sourceIndex = source.first, sourceIndex < feedTabs.count else {
            FlareLog.debug("moveFeedTab: 无效的源索引")
            return
        }

        // 获取要移动的标签
        let movedTab = feedTabs[sourceIndex]
        FlareLog.debug("moveFeedTab: 移动标签 \(movedTab.key)")
        ...
        guard sourceIndex < fullFeedIndices.count else {
            FlareLog.debug("moveFeedTab: 源索引超出范围")
            return
        }
        ...
        FlareLog.debug("moveFeedTab: 完整列表中 从索引\(fullSourceIndex) 移动到 \(fullDestIndex)")
