...
class ProfileNewRefreshViewController: UIViewController {
    ...
    @objc private func handleMoreMenuTap() {
        ...
        //    actions.data.size > 0
        // {
        //     os_log("[ProfileRefreshViewController] 条件通过 - actions.data.size: %{public}d", log: .default, type: .debug, actions.data.size)

        //     // 获取relation状态，如果是loading则使用nil，在使用时处理
        //     let relationData: UiRelation? = switch onEnum(of: state.relationState) {
        ...
        //             if let relationData = relationData {
        //                 block.relationState(relation: relationData) ?
        ...
        //         case let .mute(mute):
        //             if let relationData = relationData {
        //                 mute.relationState(relation: relationData) ?
        ...
        //                     do {
        //                         // 🔑 修复1：正确的错误处理
        //                         guard let relationData = relationData else {
        //                             os_log("[ProfileRefreshViewController] 警告：relationData为nil，无法执行操作", log: .default, type: .debug)
        //                             throw NSError(domain: "ProfileError", code: 1, userInfo: [NSLocalizedDescriptionKey: "关系状态未加载，请稍后重试"])
        //                         }

        //                         try await item.invoke(userKey: user.data.key, relation: relationData)
        ...
        //                         // 🔑 修复3：正确的成功消息
        //                         await MainActor.run {
        //                             let message = switch onEnum(of: item) {
        //                             case let .block(blockAction):
        //                                 // 操作前的状态：如果之前是屏蔽状态，操作后就是取消屏蔽
        //                                 if let relationData = relationData {
        //                                     blockAction.relationState(relation: relationData) ? "已取消屏蔽" : "已屏蔽用户"
        //                                 } else {
        //                                     "屏蔽操作完成"
        //                                 }
        //                             case let .mute(muteAction):
        //                                 // 操作前的状态：如果之前是静音状态，操作后就是取消静音
        //                                 if let relationData = relationData {
        //                                     muteAction.relationState(relation: relationData) ? "已取消静音" : "已静音用户"
        //                                 } else {
        //                                     "静音操作完成"
        //                                 }
        //                             }
        //                             ToastView(icon: UIImage(systemName: "checkmark.circle"), message: message).show()
        //                         }
        //                     } catch {
        //                         // 简单的错误处理
        //                         await MainActor.run {
        //                             ToastView(icon: UIImage(systemName: "exclamationmark.circle"), message: "操作失败").show()
        //                         }
        //                     }
        //                 }
        //             }
