...
public class GuestMastodonService(
    ...
    public suspend fun isTrendsAvailable(): Boolean =
        runCatching {
            trendsStatuses(limit = 1)
            true
        }.getOrDefault(false)

    public suspend fun isPublicTimelineAvailable(local: Boolean = false): Boolean =
        runCatching {
            publicTimeline(limit = 1, local = local.takeIf { it })
            true
        }.getOrDefault(false)
