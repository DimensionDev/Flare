...
private object GoogleWebTranslationProvider {
    suspend fun translateDocumentJson(
        ...
        translateDocument(
            document = sourceJson.decodeJson(TranslationDocument.serializer()),
            targetLanguage = targetLanguage,
    ...
    suspend fun translateBatchDocumentJson(
        ...
        translateBatchDocument(
            sourceDocument = sourceDocument,
            targetLanguage = targetLanguage,
        ).encodeJson(PreTranslationBatchDocument.serializer())

    private suspend fun translateBatchDocument(
        sourceDocument: PreTranslationBatchDocument,
        targetLanguage: String,
    ...
    private suspend fun translateBatchPayload(
        payload: PreTranslationBatchPayload,
        targetLanguage: String,
    ): PreTranslationBatchPayload =
        PreTranslationBatchPayload(
            content = payload.content?.let { translateDocument(it, targetLanguage) },
            contentWarning = payload.contentWarning?.let { translateDocument(it, targetLanguage) },
            title = payload.title?.let { translateDocument(it, targetLanguage) },
            description = payload.description?.let { translateDocument(it, targetLanguage) },
        )
