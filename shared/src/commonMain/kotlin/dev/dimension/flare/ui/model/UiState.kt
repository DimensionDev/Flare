...
inline fun <T> UiState<T>.onSuccess(action: (T) -> Unit): UiState<T> = apply {
    if (this is UiState.Success) {
        action(data)
    }
}

inline fun <T> UiState<T>.onError(action: (Throwable) -> Unit): UiState<T> = apply {
...
fun <T> CacheableState<T>.toUi(): UiState<T> {
    return data?.let {
        UiState.Success(it)
    } ?: run {
