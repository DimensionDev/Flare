...
public fun TopLevel.renderNotifications(accountKey: MicroBlogKey): List<UiTimelineV2> {
    return timeline
        ?.instructions
        ?.asSequence()
        ?.flatMap {
...
public fun Tweet.render(accountKey: MicroBlogKey): UiTimelineV2.Post {
...
public fun XQTTimeline.render(accountKey: MicroBlogKey): UiTimelineV2? {
...
public fun GetProfileSpotlightsQuery200Response.toUi(muting: Boolean): UiRelation {
...
public fun parseXQTCustomDateTime(dateTimeStr: String): Instant? {
    val months =
        mapOf(
            "Jan" to 1,
            "Feb" to 2,
...
public fun List<InstructionUnion>.list(accountKey: MicroBlogKey): List<UiList.List> =
    flatMap {
        when (it) {
...
public fun TwitterList.render(accountKey: MicroBlogKey): UiList.List {
    val user =
...
public fun InboxMessageData.renderDirectMessage(accountKey: MicroBlogKey): UiDMItem.Message {
    if (!attachment
            ?.photo
            ?.url
            .isNullOrEmpty() &&
...
public fun UserResults.render(accountKey: MicroBlogKey): UiProfile? =
    when (result) {
...
public val User.name: String
    get() = legacy.name ?: core?.name ?: "Unknown"

public val User.screenName: String
    get() = legacy.screenName ?: core?.screenName ?: "Unknown"

public val User.avatarUrl: String
    get() =
...
public fun Tweet.renderContent(
    accountKey: MicroBlogKey,
